...
pub struct TestRebalanceHandler {
    ...
    /// Stores the consumer command sender for test access (e.g., to send Seek commands)
    pub command_sender: Mutex<Option<ConsumerCommandSender>>,
}

impl TestRebalanceHandler {
    /// Returns a clone of the stored ConsumerCommandSender, if available
...
impl RebalanceHandler for TestRebalanceHandler {
    ...
    async fn async_setup_assigned_partitions(
        ...
        // Store the command sender for test access (e.g., to send Seek commands)
